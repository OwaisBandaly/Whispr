...
const Friends = () => {
  ...
  return (
    <div
      className={`md:mx-[2vw] mx-4 max-w-screen lg:p-4
      ${theme === "sunset" ? "text-violet-50" : "text-[#0f0c29]"}
    `}
    >
      <h1 className="text-3xl font-medium mb-0.5 mt-2 w-fit text-left">
        Friends
      </h1>
      <p className="mb-6 w-fit text-[.890rem] text-base-content/90">
      ...
      <div className="flex flex-wrap sm:flex-nowrap overflow-x-auto no-scrollbar justify-start bg-base-content/15 px-2 py-1 rounded-md gap-2 mb-6 max-w-fit">
        <button
          className={`px-3 py-1.5 rounded-full whitespace-nowrap font-medium text-sm flex items-center gap-2 transition-all duration-200 ${tab === "all" ? "bg-blue-600/70 text-white" : "bg-base-100"
            }`}
          onClick={() => setTab("all")}
        >
        ...
        <button
          className={`px-3 py-1.5 rounded-full whitespace-nowrap font-medium text-sm flex items-center gap-2 transition-all duration-200 ${tab === "requests"
        ...
        <button
          className={`px-3 py-1.5 rounded-full whitespace-nowrap font-medium text-sm flex items-center gap-2 transition-all duration-200 ${tab === "sent"
            ? "bg-blue-600/70 text-white"
            : "bg-base-100 text-base-content"
            }`}
          onClick={() => setTab("sent")}
            ...
            {myFriends.length === 0 ? (
              ...
              <ul className="flex flex-col gap-4">
                {myFriends.map((friend) => (
                  <li
                    key={friend._id}
                    className="flex items-start sm:items-center sm:gap-4 gap-3 p-2 border-b border-base-content/20 flex-wrap sm:flex-nowrap"
                  >
                    <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-full">
                      ...
                      <img
                        src={!imgError ? friend.profilePic : userPng}
                        alt={friend.fullName}
                    ...
                    <div className="flex flex-1 items-center justify-between">
                      <div className="flex-1">
                        <div className="font-normal text-[.950rem] sm:text-[1.10rem]">
                          {friend.fullName}
                        </div>
            ...
            {requestsLoading ? (
              ...
              <ul className="flex flex-col gap-4">
                {requests.map((req) => (
                  <li
                    key={req._id}
                    className="flex items-start sm:items-center gap-4 p-3 border-b border-base-content/20 flex-wrap sm:flex-nowrap"
                  >
                   <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-full">
                      {!imgLoaded && !imgError && (
                        <div className="absolute inset-0 flex items-center justify-center bg-base-content/10 rounded-full">
                      ...
                      <img
                        src={!imgError ? req.sender.profilePic : userPng}
                    ...
                    <div className="flex flex-1 items-center">
                      <div className="flex-1">
                        <div className="font-normal text-[.980rem] sm:text-[1.10rem]">
                          {req.sender?.fullName}
                        </div>
                        <div className="text-xs font-light text-base-content/85">
                          {req.sender?.age ? `${req.sender.age} yrs` : ""}{" "}
              ...
              <ul className="flex flex-col gap-4">
                {sent.map((req) => (
                  ...
                  >
                      ...
                      <img
                        src={!imgError ? req.recipient?.profilePic : userPng  }
                    ...
                    <div className="flex flex-1 items-center">
                      <div className="flex-1">
                        ...
                        <p className="text-xs font-light text-base-content/85">{`Sent ${timeAgo(
                          req.createdAt
                        )}`}</p>
                      </div>
